...
<html>
<body>
  <h1>Turtles & F# with Jacqueline!</h1>
...
// TODO: this should have created a document "turtles.html"
...
let save content = File.WriteAllText(path,content)
...
let pointsTriangle = 
    [
        (150.0, 50.0, 75.0, 45.0)
        (75.0, 45.0, 65.0, 150.0)
        (65.0, 150.0, 150.0, 50.0)
    ]
...
type INSTRUCTION =
    ...
    | BACKWARD of float // move backwards by x pixels
    | LEFT of float // turn left by x degrees
    | RIGHT of float // turn right by x degrees
...
let rec execute (states:State list) (program:INSTRUCTION list) =
    ...
    | currentState :: previousStates ->
        ...
        | [] -> states // Here we are matching the states on the list of states
...
createSvg initialState simpleProgram
